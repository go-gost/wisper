import{a as m,z as g,h as _,A as x,B as p,j as s,b as f,i as y,t as w}from"./index-O67Tiz1x.js";import{r as i}from"./state-Cl7TSpAo.js";import{i as l}from"./app-scaffold-DnlFs8T_.js";var k=Object.defineProperty,$=Object.getOwnPropertyDescriptor,r=(t,e,o,c)=>{for(var n=c>1?void 0:c?$(e,o):e,d=t.length-1,v;d>=0;d--)(v=t[d])&&(n=(c?v(e,o,n):v(n))||n);return c&&n&&k(e,o,n),n};const h=[{value:"system",labelKey:"settingsThemeSystem"},{value:"light",labelKey:"settingsThemeLight"},{value:"dark",labelKey:"settingsThemeDark"}],u=[{value:"en",labelKey:"settingsLangEn"},{value:"zh",labelKey:"settingsLangZh"}],b=[{value:1,labelKey:"settingsInterval1s"},{value:2,labelKey:"settingsInterval2s"},{value:5,labelKey:"settingsInterval5s"},{value:10,labelKey:"settingsInterval10s"},{value:30,labelKey:"settingsInterval30s"}];let a=class extends m{constructor(){super(...arguments),this._server="",this._entrypoint="",this._insecure=!1,this._theme="system",this._lang="en",this._statsInterval=1,this._snackbar="",this._saving=!1,this._unsubs=[]}connectedCallback(){super.connectedCallback();const t=g();this._server=t.server,this._entrypoint=t.entrypoint,this._insecure=t.insecure,this._theme=t.theme,this._lang=t.lang,this._statsInterval=t.stats_interval||1,this._unsubs.push(_(()=>{const e=g();this._server=e.server,this._entrypoint=e.entrypoint,this._insecure=e.insecure,this._theme=e.theme,this._lang=e.lang,this._statsInterval=e.stats_interval||1,this.requestUpdate()}),x(()=>this.requestUpdate()))}disconnectedCallback(){super.disconnectedCallback();for(const t of this._unsubs)t()}_navigate(t){window.history.pushState({},"",t),window.dispatchEvent(new PopStateEvent("popstate"))}_showSnackbar(t){this._snackbar=t,setTimeout(()=>{this._snackbar="",this.requestUpdate()},2e3)}async _saveSettings(){this._saving=!0;try{await p({server:this._server,entrypoint:this._entrypoint,insecure:this._insecure}),this._showSnackbar("✓ "+s("saved"))}catch{this._showSnackbar(s("saveFailed"))}this._saving=!1}async _setTheme(t){this._theme=t,this.requestUpdate(),this._showSnackbar("✓ "+s(h.find(e=>e.value===t)?.labelKey??"settingsThemeSystem"));try{await p({theme:t})}catch{}}async _setLang(t){this._lang=t,this.requestUpdate(),this._showSnackbar("✓ "+s(u.find(e=>e.value===t)?.labelKey??"settingsLangEn"));try{await p({lang:t})}catch{}}async _setInterval(t){this._statsInterval=t,this.requestUpdate(),this._showSnackbar("✓ "+s(b.find(e=>e.value===t)?.labelKey??"settingsInterval1s"));try{await p({stats_interval:t})}catch{}}_cycleOption(t,e){const o=e.indexOf(t);return e[(o+1)%e.length]}render(){return f`
      <app-scaffold>
        <!-- AppBar -->
        <div slot="appBar" style="display:flex;align-items:center;gap:8px;">
          ...
          <span class="page-title">${s("settingsTitle")}</span>
        </div>
          ...
          <div class="card">
            <div class="card-padded">
              <div class="form-group">
                <label class="form-label">${s("settingsServer")}</label>
                <input class="form-input" .value=${this._server}
                  placeholder=${s("settingsServerHint")}
                  @input=${t=>{this._server=t.target.value}}>
              </div>
              <div class="form-group">
                <label class="form-label">${s("settingsEntrypoint")}</label>
                <input class="form-input" .value=${this._entrypoint}
                  placeholder=${s("settingsEntrypointHint")}
                  @input=${t=>{this._entrypoint=t.target.value}}>
              </div>
              <div class="switch-row" style="border-bottom:none;">
                <div>
                  <div class="switch-label">${s("settingsInsecure")}</div>
                  <div class="switch-desc">${s("settingsInsecureDesc")}</div>
                </div>
        ...
        <div class="section">
          ...
          <div class="card">
            <div class="selector-row" @click=${()=>this._setLang(this._cycleOption(this._lang,u.map(t=>t.value)))}>
              <span class="selector-label">${s("settingsLanguage")}</span>
              <span class="selector-value">
                ${s(u.find(t=>t.value===this._lang)?.labelKey??"settingsLangEn")}
                ${l("chevron-right")}
              </span>
            </div>
            <div class="selector-row" @click=${()=>this._setTheme(this._cycleOption(this._theme,h.map(t=>t.value)))}>
              <span class="selector-label">${s("settingsTheme")}</span>
              <span class="selector-value">
                ${s(h.find(t=>t.value===this._theme)?.labelKey??"settingsThemeSystem")}
                ${l("chevron-right")}
              </span>
            </div>
            <div class="selector-row" @click=${()=>this._setInterval(this._cycleOption(this._statsInterval,b.map(t=>t.value)))}>
              <span class="selector-label">${s("settingsStatsInterval")}</span>
              <span class="selector-value">
                ${s(b.find(t=>t.value===this._statsInterval)?.labelKey??"settingsInterval1s")}
                ${l("chevron-right")}
              </span>
            </div>
          </div>
