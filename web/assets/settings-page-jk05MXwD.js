import{a as b,x as h,j as m,y as f,z as p,c as t,b as v,i as x,t as _}from"./index-DvIQLlSN.js";import{r as n}from"./app-scaffold-D-01mgFn.js";var y=Object.defineProperty,w=Object.getOwnPropertyDescriptor,r=(e,s,o,l)=>{for(var a=l>1?void 0:l?w(s,o):s,c=e.length-1,d;c>=0;c--)(d=e[c])&&(a=(l?d(s,o,a):d(a))||a);return l&&a&&y(s,o,a),a};const g=[{value:"system",labelKey:"settingsThemeSystem"},{value:"light",labelKey:"settingsThemeLight"},{value:"dark",labelKey:"settingsThemeDark"}],u=[{value:"en",labelKey:"settingsLangEn"},{value:"zh",labelKey:"settingsLangZh"}];let i=class extends b{constructor(){super(...arguments),this._server="",this._entrypoint="",this._insecure=!1,this._theme="system",this._lang="en",this._snackbar="",this._debounceTimer=null,this._unsubs=[]}connectedCallback(){super.connectedCallback();const e=h();this._server=e.server,this._entrypoint=e.entrypoint,this._insecure=e.insecure,this._theme=e.theme,this._lang=e.lang,this._unsubs.push(m(()=>{const s=h();this._server=s.server,this._entrypoint=s.entrypoint,this._insecure=s.insecure,this._theme=s.theme,this._lang=s.lang,this.requestUpdate()}),f(()=>this.requestUpdate()))}disconnectedCallback(){super.disconnectedCallback();for(const e of this._unsubs)e()}_navigate(e){window.history.pushState({},"",e),window.dispatchEvent(new PopStateEvent("popstate"))}_showSnackbar(e){this._snackbar=e,setTimeout(()=>{this._snackbar="",this.requestUpdate()},2e3)}_debouncedSave(e,s){this._debounceTimer&&clearTimeout(this._debounceTimer),this._debounceTimer=setTimeout(async()=>{try{await p({[e]:s}),this._showSnackbar("✓ "+t("saved"))}catch{this._showSnackbar(t("saveFailed"))}},600)}async _setTheme(e){this._theme=e,await p({theme:e}),this._showSnackbar("✓ "+t("saved"))}async _setLang(e){this._lang=e,await p({lang:e}),this._showSnackbar("✓ "+t("saved"))}_cycleOption(e,s){const o=s.indexOf(e);return s[(o+1)%s.length]}render(){return v`
      <app-scaffold>
        <!-- AppBar -->
          ...
          <div class="detail-card">
            <div class="card-padded">
              ...
              <div class="switch-row">
                ...
                <div class="switch ${this._insecure?"on":""}" @click=${()=>{this._insecure=!this._insecure,this._debouncedSave("insecure",this._insecure)}}>
        ...
        <div class="detail-section">
          <div class="detail-card">
            <div class="selector-field" @click=${()=>this._setLang(this._cycleOption(this._lang,u.map(e=>e.value)))}>
              <span class="selector-label">${t("settingsLanguage")}</span>
              <span class="selector-value">${t(u.find(e=>e.value===this._lang)?.labelKey??"settingsLangEn")} ▶</span>
            </div>
            <div class="selector-field" @click=${()=>this._setTheme(this._cycleOption(this._theme,g.map(e=>e.value)))}>
              <span class="selector-label">${t("settingsTheme")}</span>
              <span class="selector-value">${t(g.find(e=>e.value===this._theme)?.labelKey??"settingsThemeSystem")} ▶</span>
            </div>
          </div>
        </div>

        ${this._snackbar?v`<div class="toast">${this._snackbar}</div>`:""}
      </app-scaffold>
    `}};i.styles=x`
    /* ── AppBar ── */
    .back-btn {
