...
// NewTCPEntryPoint creates a TCP entrypoint.
...
func (s *tcpEntryPoint) ID() string   { return s.opts.ID }
...
func (s *tcpEntryPoint) StatsBaseline() cfg.ServiceStats {
	s.mu.RLock()
	defer s.mu.RUnlock()
	return s.statsBaseline
}

func (s *tcpEntryPoint) SetStatsBaseline(baseline cfg.ServiceStats) {
	s.mu.Lock()
	defer s.mu.Unlock()
