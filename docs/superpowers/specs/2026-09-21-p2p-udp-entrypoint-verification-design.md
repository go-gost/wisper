...
| Run | 配置 | 观测点 |
|---|---|---|
| R1 | 进程内 provider 原样，1 客户端 | 是否通；回包是否带 2 字节前缀；outlet/p2p host 日志中的解析错误 → 证实/证伪 framing 缺陷 |
| R2 | 加**测试内 framing shim**（用 `x/p2p/streamconn.New` 把 provider conn 包成 udp framed，模拟插件路径的 conn），1 客户端 | 数据面正确基线；证明 R1 的失败归因于 framing 而非其他 |
| R3 | 同 R2，2 个并发客户端（不同源端口、不同 payload） | last-dial-wins 签名：client 1 的隧道是否被 client 2 顶掉；回复是否错投；两侧日志 |

判定规则：R2 若也不通，说明 shim 假设错误 → 停止、回到静态分析重新定位（不硬写断言）。

## 产出

- 可复跑测试：R1 断言**实测签名**并注明「修复落地后应反转」；R2/R3 断言实测行为。
- [p2p-integration.md](../../../docs/p2p-integration.md) 「已知限制」一节改写：实测签名 +
  ...
  或 GOST 侧 session 多路复用，对齐 relay 协议的 udp session id 思路）。
- 产品代码零改动。
